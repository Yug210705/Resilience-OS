...
class RecoveryPlanRecord(Base):
    ...
    id = Column(String(255), primary_key=True, default=lambda: f"RP-{str(uuid.uuid4())[:8].upper()}")
    disruption_id = Column(String(255), nullable=False, index=True)
    scenario_id = Column(String(255), nullable=True, index=True)
    strategy = Column(String(255), nullable=False) # e.g., "Activate SUP-004"
    ...
    status = Column(String(255), default="PENDING_AUDIT", nullable=False, index=True)
...
class ScenarioRecord(Base):
    ...
    id = Column(String(255), primary_key=True, default=lambda: f"SCN-{str(uuid.uuid4())[:8].upper()}")
    ...
    status = Column(String(255), default="READY", nullable=False, index=True)
