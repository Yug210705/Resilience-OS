...
export default function RiskWorkspace() {
  ...
  const loadData = useCallback(async (currentPage: number, currentSearch: string) => {
    ...
    try {
      ...
      setError(e.message || 'Unable to load risk review queue.');
    } finally {
      setLoading(false);
    }
  }, [selectedPlanId]);
      ...
      <div className="flex-1 flex flex-col min-h-0 space-y-6">
        ...
        <div className="shrink-0 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
          <div>
            <div className="flex items-center space-x-2 text-[13px] text-slate-500 mb-2 font-medium">
              ...
              <span className="text-slate-900">Risk & Audit</span>
            </div>
            <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Risk & Audit Review Queue</h1>
            <p className="text-[13px] text-slate-500 mt-1">Review and validate operational policies for pending recovery plans.</p>
          </div>
        ...
        {!data && loading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-white border border-slate-200 rounded-xl shadow-sm py-32 text-center">
            ...
            <h2 className="text-[15px] font-semibold text-slate-900">No recovery plans currently require risk review.</h2>
            <p className="text-[13px] text-slate-500 mt-1.5 max-w-sm mx-auto">All active recovery plans have passed the audit phase.</p>
          </div>
        ) : (
          <div className="flex-1 flex flex-col min-h-0 space-y-6">
              ...
              <div className="xl:col-span-2 bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col h-full min-h-0">
                <div className="shrink-0 px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white rounded-t-xl">
                  <div className="flex items-center space-x-2">
                    <div className="text-[13px] font-medium text-slate-600 bg-slate-100 px-3 py-1.5 rounded-md">Pending Audit Queue</div>
                  </div>
